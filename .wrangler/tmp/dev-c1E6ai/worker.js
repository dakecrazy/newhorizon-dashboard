...
// .wrangler/tmp/bundle-iyP7wZ/strip-cf-connecting-ip-header.js
...
var require_bn = __commonJS({
  "node_modules/bn.js/lib/bn.js"(exports, module) {
    ...
    (function(module2, exports2) {
      ...
      BN3.prototype.toString = /* @__PURE__ */ __name(function toString(base2, padding) {
        base2 = base2 || 10;
        padding = padding | 0 || 1;
        var out;
...
// .wrangler/tmp/bundle-iyP7wZ/middleware-loader.entry.ts
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-iyP7wZ/middleware-insertion-facade.js
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
Signature.prototype._importDER = /* @__PURE__ */ __name(function _importDER(data2, enc) {
...
async function importTextSecret(key2, algorithm, keyUsages) {
  return await crypto.subtle.importKey("raw", textToUint8Array(key2), algorithm, true, keyUsages);
}
__name(importTextSecret, "importTextSecret");
async function importJwk(key2, algorithm, keyUsages) {
  return await crypto.subtle.importKey("jwk", key2, algorithm, true, keyUsages);
}
__name(importJwk, "importJwk");
async function importPublicKey(key2, algorithm, keyUsages) {
  return await crypto.subtle.importKey("spki", pemToBinary(key2), algorithm, true, keyUsages);
}
__name(importPublicKey, "importPublicKey");
async function importPrivateKey(key2, algorithm, keyUsages) {
  return await crypto.subtle.importKey("pkcs8", pemToBinary(key2), algorithm, true, keyUsages);
}
__name(importPrivateKey, "importPrivateKey");
async function importKey(key2, algorithm, keyUsages) {
  if (typeof key2 === "object")
    return importJwk(key2, algorithm, keyUsages);
  if (typeof key2 !== "string")
    throw new Error("Unsupported key type!");
  if (key2.includes("PUBLIC"))
    return importPublicKey(key2, algorithm, keyUsages);
  if (key2.includes("PRIVATE"))
    return importPrivateKey(key2, algorithm, keyUsages);
  return importTextSecret(key2, algorithm, keyUsages);
}
...
async function sign3(payload, secret, options = "HS256") {
  if (typeof options === "string")
  ...
  if (options.algorithm === "none")
    return partialToken;
  const key2 = secret instanceof CryptoKey ? secret : await importKey(secret, algorithm, ["sign"]);
  const signature2 = await crypto.subtle.sign(algorithm, key2, textToUint8Array(partialToken));
  return `${partialToken}.${arrayBufferToBase64Url(signature2)}`;
}
__name(sign3, "sign");
async function verify3(token, secret, options = "HS256") {
  if (typeof options === "string")
    options = { algorithm: options };
  ...
  try {
    ...
    if (algorithm.name === "none")
      return decodedToken;
    const key2 = secret instanceof CryptoKey ? secret : await importKey(secret, algorithm, ["verify"]);
    if (!await crypto.subtle.verify(algorithm, key2, base64UrlToUint8Array(tokenSignature), textToUint8Array(`${tokenHeader}.${tokenPayload}`)))
      throw new Error("INVALID_SIGNATURE");
    return decodedToken;
  } catch (err) {
...
var worker_default = {
  async fetch(req, env) {
    ...
    if (url.pathname === "/challenge")
      return challenge(req, env);
    if (url.pathname === "/update")
      return update2(req, env);
    if (url.pathname === "/verify")
      return verify4(req, env);
...
async function challenge(req, env) {
  ...
  const key2 = `nonce:${address.toLowerCase()}`;
  await env.KV.put(key2, nonce, { expirationTtl: 300 });
  const message = `Equity Dashboard Login
Nonce: ${nonce}`;
  return Response.json({ ok: true, message });
}
__name(challenge, "challenge");
async function verify4(req, env) {
  const { address, signature: signature2, message } = await req.json();
...
Nonce: ${nonce}`;
  ...
  let recovered;
  try {
    recovered = verifyMessage(message, signature2);
  } catch {
    return Response.json({ ok: false }, { status: 401 });
  }
...
async function data(req, env) {
  ...
  if (!token || !await index_default.verify(token, env.JWT_SECRET)) {
    return new Response("Unauthorized", { status: 401 });
  }
